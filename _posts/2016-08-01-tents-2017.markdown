...
for (var j = 0; j < tableHeaderElements.length; j++) {
    if (tableHeaderElements[j].innerHTML.indexOf(searchString) !== -1) {
        tableHeaderElements[j].innerHTML=tableHeaderElements[j].innerHTML.replace(existingTitle1,newTitle1);          break;
    }
}
</script>
